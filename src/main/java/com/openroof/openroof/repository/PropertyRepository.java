...
public interface PropertyRepository extends JpaRepository<Property, Long>, JpaSpecificationExecutor<Property> {
    ...
    @Query("SELECT p FROM Property p WHERE p.status = :status AND p.visibility = :visibility AND p.trashedAt IS NULL AND p.deletedAt IS NULL ORDER BY p.highlighted DESC NULLS LAST, p.createdAt DESC")
